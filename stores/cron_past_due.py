...
def send_past_due_notification():
    ...
    for subscription in subscriptions:
        try:
            ...
            logging.info(msg)
            send_mail('Past due Subscription', msg, 'no-reply@greatcoins.com', [b for (a,b) in settings.ADMINS], fail_silently=True)
            ...
            send_mail('Past due Suscription', msg, 'admin@greatcoins.com',  [local_subscription.owner.user.email], fail_silently=True)
            ...
            send_mail('Subscription not found', error_msg , 'admin@greatcoins.com', [b for (a,b) in settings.ADMINS], fail_silently=True)
        
        except Exception, e:
            send_mail('Error when trying to check past due subscriptions', e , 'admin@greatcoins.com', [b for (a,b) in settings.ADMINS], fail_silently=True)
